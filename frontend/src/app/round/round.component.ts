...
import { Competition, competitionSelector, Game, gameSelector, IAppState, Player, playerSelector, Round, roundSelector, RoundStatus, StandingLine, standingLineSelector } from '../store';
...
export class RoundComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.store.select(roundSelector).pipe(
      ...
      tap(round => {
        ...
        this.store.dispatch(
          new standingLineActions.Get(this.selectedClubKey, this.selectedRound.competitionKey, this.selectedRound.roundNumber));
        this.store.dispatch(
          new gameActions.GetAll(this.selectedClubKey, this.selectedRound.competitionKey, this.selectedRound.key));
        ...
        this.store.select(standingLineSelector).pipe(select(s => s.data),
          tap(standingLines => {
            this.standingLines = standingLines.filter(x => x.competitionKey === this.selectedRound.competitionKey
        ...
        this.store.select(gameSelector).pipe(select(g => g.data),
          tap(games => {
            this.competitionGames = games.filter(g => g.competitionKey === this.competition.key);
            this.roundGames = games.filter(g => g.competitionKey === this.competition.key
  ...
  generateGames(): void {
    ...
    playersInRound.forEach(player => {
      if (!this.standingLines.find(x => x.playerKey === player.key)) {
        ...
        this.store.dispatch(new standingLineActions.Create(standingLine, this.selectedClubKey, this.competitionKey));
      }
    });
  ...
  private makeGame(player1: string, player2: string) {
    ...
    const player1Whites = this.competitionGames.filter(g => g.whitePlayerKey === player1).length;
    const player1Blacks = this.competitionGames.filter(g => g.blackPlayerKey === player1).length;
    const player1Diff = player1Whites - player1Blacks;
    const player2Whites = this.competitionGames.filter(g => g.whitePlayerKey === player2).length;
    const player2Blacks = this.competitionGames.filter(g => g.blackPlayerKey === player2).length;
    ...
    // create game
    this.store.dispatch(new gameActions.Create(game, this.selectedClubKey, this.competitionKey, this.selectedRound.key));
  }
  ...
  private sortStandingLines(list: StandingLine[]) {
    return list.sort((left, right): number => {
      if (+left.percentage < +right.percentage) { return 1; }
      if (+left.percentage > +right.percentage) { return -1; }
      return 0;
    });
  }
  ...
  public finishRound() {
    ...
    this.roundGames.forEach((game) => {
      ...
      whiteStandingLine.percentage = (whiteStandingLine.points / whiteStandingLine.gamesPlayed) * 100;
      blackStandingLine.percentage = (blackStandingLine.points / blackStandingLine.gamesPlayed) * 100;
  ...
  public currentRound(): Game[] {
    return this.competitionGames.filter(x => x.roundNumber === this.selectedRound.roundNumber &&
  ...
  public allGamesFinished(): boolean {
    if (this.roundGames.length > 0 && this.roundGames.filter(x => x.result === null || x.result === undefined).length === 0) {
      return true;
    }
    return false;
  }
  ...
  public cancelRound(): void {
    this.roundGames.forEach((game) => {
      this.store.dispatch(new gameActions.Delete(game.key, this.selectedClubKey, this.competitionKey, this.selectedRound.key));
    });
