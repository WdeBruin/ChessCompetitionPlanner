...
export class RoundComponent implements OnInit {
    ...
    processResult(game: Game, result: number) {
        let whitePlayer = this.players.find(x => x.key == game.whitePlayerKey);
        let whiteStandingLine = this.standingLines.find(x => x.playerKey == game.whitePlayerKey);
        let blackPlayer = this.players.find(x => x.key == game.blackPlayerKey);
        let blackStandingLine = this.standingLines.find(x => x.playerKey == game.blackPlayerKey);
    ...
    generateGames(): void {
        ...
        let playerIds = playersInRound.map(x => x.key);
        ...
        while (standingLines.length > 1) {
            ...
            let candidates = standingLines.filter(x => x.playerKey != line.playerKey);
    ...
    toggle(playerKey: string) {
        ...
        let index = this.roundplayerKeys.indexOf(playerKey);
    ...
    private makeGame(player1: string, player2: string) {
        ...
        let player1Whites = this.games.filter(g => g.whitePlayerKey == player1).length;
        let player1Blacks = this.games.filter(g => g.blackPlayerKey == player1).length;
        let player1Diff = player1Whites - player1Blacks;
        let player2Whites = this.games.filter(g => g.whitePlayerKey == player2).length;
        let player2Blacks = this.games.filter(g => g.blackPlayerKey == player2).length;
        ...
        let whitePlayer = this.players.find(p => p.key == game.whitePlayerKey);
        let blackPlayer = this.players.find(p => p.key == game.blackPlayerKey);
    ...
    public getName(playerKey: string): string {
        let player = this.players.find(x => x.key == playerKey);
        if(player) {
